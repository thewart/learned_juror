...
transformed parameters {
  ...
  for (i in 1:N) {
    ...
    eta[i] = alpha + Z[i]*(mu_lambda[Cond[i]][1] + lambda_subj[Subj[i]][1]) + X[i]*beta + sum(X[i])*Z[i]*(mu_lambda[Cond[i]][2] + lambda_subj[Subj[i]][2]);
...
generated quantities {
  ...
  real mu_alpha_pre_resp[Ncond];
  real mu_alpha_post_resp[Ncond];
  real mu_beta_ave_pre_resp[Ncond];
  ...
  for (c in 1:Ncond) {
    mu_alpha_pre_resp[c] = Phi(mu_alpha[c]/exp(mu_scale + pow(sigma_scale,2)/2))*(U-L) + L;
    mu_alpha_post_resp[c] = Phi((mu_alpha[c] + mu_lambda[c][1])/exp(mu_scale + pow(sigma_scale,2)/2))*(U-L) + L;
    mu_beta_ave_pre_resp[c] = (Phi(mean(mu_beta[c])/exp(mu_scale + pow(sigma_scale,2)/2)) - 0.5)*(U-L) + L;
