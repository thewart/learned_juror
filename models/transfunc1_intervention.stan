...
model {
  ...
  for (i in 1:N) {
    ...
    vector[2] lambda = mu_lambda + lambda_subj[i];
...
generated quantities {
  ...
  real mu_lambda_alpha = Phi(mu_lambda[1]/exp(mu_scale + pow(sigma_scale,2)/2))*(U-L) + L;
  real mu_lambda_beta = Phi(mu_lambda[2]/exp(mu_scale + pow(sigma_scale,2)/2))*(U-L) + L;
  real<lower=0,upper=1> rho_lambda = Omega_lambda[2,1];
